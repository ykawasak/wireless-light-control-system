...
void HAL_ETH_MspDeInit(ETH_HandleTypeDef *heth) {
	if (heth->Instance == ETH) {
		...
		HAL_GPIO_DeInit(GPIOC,
				MII_MDC_Pin | MII_TXD2_Pin | MII_TX_CLK_Pin | MII_RXD0_Pin
						| MII_RXD1_Pin);
...
void HAL_LTDC_MspInit(LTDC_HandleTypeDef *hltdc) {
	...
	if (hltdc->Instance == LTDC) {
		...
		GPIO_InitStruct.Pin = LCD_DISP_Pin | GPIO_PIN_14 | GPIO_PIN_12
				| GPIO_PIN_13 | GPIO_PIN_11 | GPIO_PIN_10 | GPIO_PIN_9
				| GPIO_PIN_0 | GPIO_PIN_8 | GPIO_PIN_7 | GPIO_PIN_6 | GPIO_PIN_1
...
void HAL_LTDC_MspDeInit(LTDC_HandleTypeDef *hltdc) {
	if (hltdc->Instance == LTDC) {
		...
		HAL_GPIO_DeInit(GPIOJ,
				LCD_DISP_Pin | GPIO_PIN_14 | GPIO_PIN_12 | GPIO_PIN_13
...
void HAL_QSPI_MspDeInit(QSPI_HandleTypeDef *hqspi) {
	if (hqspi->Instance == QUADSPI) {
		...
		HAL_GPIO_DeInit(GPIOF,
				GPIO_PIN_6 | GPIO_PIN_7 | GPIO_PIN_10 | GPIO_PIN_9);
...
void HAL_MMC_MspInit(MMC_HandleTypeDef *hmmc) {
	...
	if (hmmc->Instance == SDMMC1) {
		...
		/**SDMMC1 GPIO Configuration
		 ...
		 PC6     ------> SDMMC1_D6
		 */
		GPIO_InitStruct.Pin = SDIO1_D2_Pin | SDIO1_D3_Pin | SDIO1_CK_Pin
				| SDIO1_D0_Pin | SDIO1_D1_Pin | SDIO1_D7_Pin | SDIO1_D6_Pin;
		GPIO_InitStruct.Mode = GPIO_MODE_AF_PP;
		GPIO_InitStruct.Pull = GPIO_NOPULL;
...
void HAL_MMC_MspDeInit(MMC_HandleTypeDef *hmmc) {
	if (hmmc->Instance == SDMMC1) {
		 ...
		 */
		HAL_GPIO_DeInit(GPIOC,
				SDIO1_D2_Pin | SDIO1_D3_Pin | SDIO1_CK_Pin | SDIO1_D0_Pin
...
void HAL_PCD_MspDeInit(PCD_HandleTypeDef *hpcd) {
	if (hpcd->Instance == USB_OTG_FS) {
		...
		HAL_GPIO_DeInit(GPIOA,
				VBUS_FS2_Pin | USB_OTG_FS2_P_Pin | USB_OTG_FS2_N_Pin);
...
void HAL_SAI_MspDeInit(SAI_HandleTypeDef *hsai) {
	...
	if (hsai->Instance == SAI2_Block_A) {
		 ...
		 */
		HAL_GPIO_DeInit(GPIOI,
				GPIO_PIN_6 | GPIO_PIN_5 | GPIO_PIN_4 | GPIO_PIN_7);
